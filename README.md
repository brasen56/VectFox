...
- 🧩 **InlineSummary-compatible extraction.** Collapsed InlineSummary messages are automatically expanded from their stored originals—including nested and reference-backed summaries—before EventBase extraction. Auto-sync markers, retrieval recency, and prompt ghosting use the same expanded timeline, so pausing auto-sync does not force a coordinate reset.
