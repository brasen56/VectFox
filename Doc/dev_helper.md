...
| `eventbase_recency_source` (`index` / `story_time`)                         | `index`                   | Recency term of the re-rank formula                            | `story_time` decays by narrative-clock distance (event `DateTime` vs story "now" parsed from recent messages, floored by max candidate DateTime; half-life = 20% of candidate span, floor 1 story-day — see `core/story-time.js`). Immune to ILS-expanded/collapsed index drift and summary flattening. Forces the JS re-rank path (suppresses `eventbase_native_rerank` — the Qdrant formula's recency is `source_window_end`-hardwired). Events without a parseable `DateTime` score neutral 0.5. |
